...
func createDB() {
	sqlStmt := "CREATE TABLE Execution (id INTEGER, name VARCHAR(255), status VARCHAR(50), start DATETIME, end DATETIME);"
	if db == nil {
...
func RepoFindExec(id int) Exec {
	...
	var exec Exec
	for rows.Next() {
		err2 := rows.Scan(&exec.Id, &exec.Name, &exec.Status, &exec.StartDate, &exec.EndDate)
		if err2 != nil {
			log.Fatal(err2)
		}
	}
...
func addExec(exec Exec) Exec{
	...
	_, err2 := stmt.Exec(exec.Id, exec.Name, exec.Status, exec.StartDate, exec.EndDate)
	if err2 != nil {
		log.Fatal(err2)
	}
	return exec
